...
const { indicePorProducto, indiceDeProduccion } = require("./queries/analisis");
const {
  obtenerRegistrosPE,
...
const resolvers = {
  ...
  Query: {
    ...
    obtenerStockEsponjas: async () => {
      ...
      stockProductos.forEach(function (loteProducto) {
        listaProductos.forEach(function (producto) {
          if (
            producto.id == loteProducto.producto &&
            producto.categoria == "Esponjas" &&
            loteProducto.estado != "Terminado"
          ) {
            lotesEsponjas.push({
    ...
    obtenerProductosTotal: async () => {
      ...
      productos.forEach(function (producto) {
        if (
          stockProducto.find(
            (i) => i.producto == producto.id && i.estado === "Terminado"
          )
        ) {
          respuesta.push({
            id: producto.id,
        ...
        if (
          stockProducto.find(
            (i) => i.producto == producto.id && i.estado === "Proceso"
          )
        ) {
          respuesta.push({
            id: producto.id,
        ...
        if (
          stockProducto.find(
            (i) => i.producto == producto.id && i.estado === "Reproceso"
          )
        ) {
          respuesta.push({
            id: producto.id,
    ...
    obtenerUltimosModificados: async () => {
      const lote = await StockProducto.find({
        creado: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) },
      })
        .sort({ $natural: -1 })
        .limit(10);
      return lote;
  ...
  Mutation: {
    ...
    nuevoProductoStock: async (_, { input }) => {
      ...
      const existeLote = await StockProducto.findOne({
        lote: lote,
        estado: estado,
        producto: producto,
      });
      if (existeLote) {
        ...
        nuevoLote = await StockProducto.findByIdAndUpdate(
          { _id: existeLote.id },
          nuevoInput,
          { new: true }
        );
    ...
    actualizarProductoStock: async (_, { id, input }) => {
      ...
      lote = await StockProducto.findByIdAndUpdate({ _id: id }, input, {
        new: true,
      });
    ...
    actualizarInsumo: async (_, { id, input }) => {
      ...
      insumo = await Insumo.findByIdAndUpdate({ _id: id }, input, {
        new: true,
      });
    ...
    actualizarInsumoStock: async (_, { id, input }) => {
      ...
      insumo = await StockInsumo.findByIdAndUpdate({ _id: id }, input, {
        new: true,
      });
      ...
      for (let index = 0; index < lotes.length; index++) {
        ...
        try {
          if (lote && lote.cantidad < lotes[index].cantidad) {
            if (lote) {
              throw new Error(
                `No hay disponibilidad de producto en el Lote: ${lote.lote}.`
              );
            } else {
        ...
        try {
          if (lote && lote.cantidad >= lotes[index].cantidad) {
            ...
            if (lote.cantidad > lotes[index].cantidad) {
              ...
              await StockProducto.findByIdAndUpdate(
                { _id: lotes[index].loteID },
                lote,
                { new: true }
              );
            } else {
    ...
    actualizarRegistroSalida: async (_, { id, input }) => {
      ...
      //Actualizar DB
      registro = await Salida.findByIdAndUpdate({ _id: id }, input, {
        new: true,
      });
      ...
      if (lote) {
        ...
        try {
          if (existeInsumo) {
            ...
            await StockInsumo.findByIdAndUpdate(
              { _id: existeInsumo.id },
              existeInsumo,
              { new: true }
            );
    ...
    actualizarRegistroIngreso: async (_, { id, input }) => {
      ...
      //Actualizar DB
      registro = await Ingreso.findByIdAndUpdate({ _id: id }, input, {
        new: true,
      });
    ...
    nuevoRegistroPP: async (_, { id, input }) => {
      ...
      try {
        if (id) {
          ...
          input.modificado = Date.now();
          input.estado = false;
          resultado = await CPP.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
    ...
    actualizarRegistroPP: async (_, { id, input }) => {
      ...
      await StockInsumo.findByIdAndUpdate({ _id: regPlaca.id }, regPlaca, {
        new: true,
      });
      await StockInsumo.findByIdAndUpdate({ _id: regTapon.id }, regTapon, {
        new: true,
      });

      regLotes.forEach(async function (reg) {
        reg.lote = lote;
        await StockProducto.findByIdAndUpdate({ _id: reg.id }, reg, {
          new: true,
        });
        switch (reg.estado) {
          case "Proceso":
    ...
    nuevoRegistroGP: async (_, { id, input }) => {
      ...
      try {
        ...
        if (id) {
          ...
          input.estado = false;
          resultado = await CGP.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
    ...
    nuevoRegistroPE: async (_, { id, input }) => {
      ...
      try {
        ...
        if (id) {
          ...
          input.estado = false;
          input.modificado = finalizar;
          resultado = await CPE.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
          const iniciar = Date.now();
    ...
    actualizarRegistroStockPE: async (_, { id, input }) => {
      const { lote, cantProducida, descarte } = input;
      ...
      const regLotes = await StockProducto.find({ lote: registro.lote });
      const regLote = await StockProducto.findOne({
        lote: registro.lote,
        estado: "Proceso",
      });
      ...
      await StockInsumo.findByIdAndUpdate({ _id: regBolsa.id }, regBolsa, {
        new: true,
      });
      await StockInsumo.findByIdAndUpdate({ _id: regEsponja.id }, regEsponja, {
        new: true,
      });

      // Si el Lote de producto continua en Proceso modificarlo
      regLote.cantidad += difCantidadProducida;
      await StockProducto.findByIdAndUpdate({ _id: regLote.id }, regLote, {
        new: true,
      });
      ...
      if (registro.lote !== lote) {
        if (regLotes.length > 1) {
          regLotes.forEach(async function (l) {
            ...
            await StockProducto.findByIdAndUpdate({ _id: l.id }, l, {
              new: true,
            });
          });
        } else {
    ...
    nuevoRegistroGE: async (_, { id, input }) => {
      ...
      try {
        ...
        if (id) {
          ...
          input.modificado = finalizar;
          input.estado = false;
          resultado = await CGE.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
          const iniciar = Date.now();
    ...
    nuevoRegistroSP: async (_, { id, input }) => {
      ...
      let loteReproceso = await StockProducto.findOne({
        lote: lote,
        estado: "Reproceso",
      });
      try {
        ...
        if (id) {
          ...
          input.modificado = Date.now();
          input.estado = false;
          resultado = await CSP.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
          input.creado = Date.now();
    ...
    nuevoRegistroPG: async (_, { input }) => {
      ...
      try {
        ...
        await StockInsumo.findByIdAndUpdate({ _id: loteInsumoID }, loteGel, {
          new: true,
        });
    ...
    nuevoRegistroCPG: async (_, { id, input }) => {
      ...
      let loteStock = await StockProducto.findOne({
        lote: lote,
        producto: productoID,
      });
      try {
        if (id) {
          ...
          input.modificado = Date.now();
          input.estado = false;
          resultado = await CPG.findByIdAndUpdate({ _id: id }, input, {
            new: true,
          });
        } else {
    ...
    nuevoDobleRegistroCPG: async (_, { id, input }) => {
      ...
      // Obtener informacion en Insumos y Productos en Stock
      let bolsaStock = await StockInsumo.findById(loteBolsaID);
      let bolsaCristalStock = await StockInsumo.findOne({
        lote: loteBolsaCristal,
      });
      ...
      try {
        if (id) {
          if (!loteBolsaCristal) {
            ...
            if (!bolsaStock) {
              bolsaStock = await StockInsumo.findOne({ lote: loteBolsa });
            }
            // Actualizar la cantidad de Insumo de Bolsa simple
            await StockInsumo.findByIdAndUpdate(
              { _id: bolsaStock.id },
            ...
            bolsaCristalStock.cantidad -=
              cantProducida + cantDescarteBolsaCristal;
            await StockInsumos.findByIdAndUpdate(
              { _id: bolsaCristalStock.id },
              bolsaCristalStock,
              { new: true }
            );
          ...
          if (loteStock) {
            const primerReg = await CPG.findOne({
              lote: loteStock.lote,
              estado: false,
            });
            if (primerReg) {
